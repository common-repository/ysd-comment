...
if(!empty($_GET['ysd_a'])){
	...
	add_action('comment_form_after','ysd_comment_form_alert');
...
function webo_login(){
	...
	echo '<script>function login_success(name){ document.getElementById("author").value=name;document.getElementById("commentform").onsubmit=function(){return true;} }</script>';
...
function ysd_comment_form_alert(){
	if(!is_user_logged_in() && !isset($_SESSION['ysd_sso_ticket'])){
		echo<<<JS
		<script>
